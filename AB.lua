	...
	if CiazwareUniversalAimbotLoaded == true then
		Notify("TVN HUB", "Script Loaded Already", "", 3)
		return 
	end
    ...
    if workspace and workspace:FindFirstChild"Teams" then 
        if getgenv().TeamCheck == true then
            if #workspace.Teams:GetChildren() == 0 then 
                ...
                SeparateNotify("TVN HUB", "TeamCheck set to: "..tostring(getgenv().TeamCheck).." because there are no teams!", "", 3)
            end
        end
    end
end
...
--[[getgenv().GetNearestTarget = function()
			...
			if Key.UserInputType == Enum.UserInputType.MouseButton2 then 
				pcall(function()
					...
					if Target ~= nil then 
						...
						Notify("TVN HUB", "Aimlock Target: "..tostring(AimlockTarget), "", 3)
					end
				end)
			end
			if Key.KeyCode == Enum.KeyCode[AimlockToggleKey] then 
				Aimlock = not Aimlock
				Notify("TVN HUB", "Aimlock: "..tostring(Aimlock), "", 3)
			end
		end
	end)
	...
	SeparateNotify("TVN HUB", "Universal Aimbot loaded in: "..string.format("%.7f", tostring(tick() - CiazwareUniversalAimbotLoadingTime)), "", 3)
